class SearchController < ApplicationController
  ...
  def do_search(query)
    ...
    results = ActiveRecord::Base.connection.select_all("select product_id as product_id from search_terms where term in (#{words.join(', ')}) group by product_id order by sum(\"count\") desc limit 50")
